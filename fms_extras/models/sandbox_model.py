...
class SandboxUnit(nn.Module):
    def __init__(
        ...
        self.gscale = nn.Parameter(torch.rand(hidden_dim))
    ...
    def reset_parameters(self, gain=1.0):
        ...
        # (q / sqrt2) * (v+v) * wout
        # Plugging in:
        # x sqrtd / sqrt2 * x sqrtd sqrt2 * x sqrtd sqrtg
        # Set to gain, solve for x
        # x**3 (sqrt1 * sqrtg * d**1.5) = target gain
        # x = (gain * sqrt1 / d**1.5 / sqrtg)**(1/3)
        for layer in [self.w_in.weight, self.w_out.weight]:  # , self.conv, self.mulbias]:
            nn.init.normal_(
                layer,
                mean=0.0,
                std = (gain * 1**.5 / self.hidden_grow_factor**.5 / self.width**1.5)**(1/3)
            )
        self.bias.data.random_()
        self.gscale.data.random_()
        if self.use_bias:
    ...
    def forward(self, x):
        ...
        c = c.sub(c.min())
        c = c.div(c.max()).add(1)
        g = g.mul(c).add(b).sigmoid()
        
        # Scan
        z = self.scan(v, g).add(v)

        # Out project / add
        # return residual + self.w_out(z * q)
        return residual * (1 - 1/self.layer_id)**.5 + (1/self.layer_id)**.5 * self.w_out(z * q)
